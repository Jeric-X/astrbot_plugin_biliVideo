...
class BiliVideoPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict):
        ...
        self._log(
            ...
            "bilibili_subscription_list 工具供 AI 调用"
        )
    ...
    async def _check_and_send_new_videos(
        self,
        origin: str,
        send_callback=None,
    ) -> dict:
        """
        检查订阅UP主是否有新视频，有则生成总结并发送。

        :param origin: unified_msg_origin
        :param send_callback: 异步回调 async (notify_text, note_render_result) -> None
                              notify_text: str  通知文本
                              note_render_result: list[Image] 或 str  渲染后的总结
                              为 None 时不发送，只返回结构化数据
        :return: 结构化结果 dict
        """
        subs = self.subscription_mgr.get_subscriptions(origin)
        if not subs:
            ...
            try:
                ...
                if send_callback is not None:
                    try:
                        ...
                        notify_text = (
                            f"\U0001f514 UP主【{up.get('name', '')}】有新视频!\n"
                            f"\U0001f4fa {latest.get('title', '')}\n"
                            f"\u23f3 正在生成总结..."
                        )
                        note = await self._generate_note(video_url)
                        note_result = self._render_and_get_chain(note)
                        await send_callback(notify_text, note_result)
                    except Exception as push_err:
                        logger.error(f"[_check_and_send_new_videos] 发送总结失败: {push_err}")

                await asyncio.sleep(1)
            except Exception as e:
                logger.error(f"检查UP主 {up.get('name', '?')} 失败: {e}")
